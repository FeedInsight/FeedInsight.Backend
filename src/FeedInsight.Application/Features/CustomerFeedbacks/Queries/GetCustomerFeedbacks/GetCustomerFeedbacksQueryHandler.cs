...
public class GetCustomerFeedbacksQueryHandler: IRequestHandler<GetCustomerFeedbacksQuery, ErrorOr<PaginatedResult<CustomerFeedbackDto>>>
{
    ...
    public async Task<ErrorOr<PaginatedResult<CustomerFeedbackDto>>> HandleAsync(
        GetCustomerFeedbacksQuery request,
        CancellationToken cancellationToken = default)
    {
        ...
        if (!feedbacks.Any())
        {
            return new PaginatedResult<CustomerFeedbackDto>(
                new List<CustomerFeedbackDto>(),
                totalCount);
        }
        ...
        var extractedTasks = await _extractedTaskRepository.ListAsync(
            new ExtractedTasksByFeedbackIdsSpec(feedbackIds),
            cancellationToken);

       
        var categoryIds = extractedTasks
        ...
        var tasksByFeedback = extractedTasks
            .GroupBy(t => t.CustomerFeedbackId)
            .ToDictionary(
                g => g.Key,
                g => g.ToList());
        ...
        var items = feedbacks
            .Select(f => new CustomerFeedbackDto(
                ...
                f.CreatedAt,
                tasksByFeedback.TryGetValue(f.Id, out var tasks)
                    ? tasks.Select(t => new ExtractedTaskDto(
                        t.Id,
                        t.CategoryId,
                        categoryLookup.GetValueOrDefault(t.CategoryId),
                        t.UserStoryId,
                        t.ExtractedIntent,
                        t.TechnicalKeywords,
