...
public class SyncUserStoryToJiraCommandHandler : IRequestHandler<SyncUserStoryToJiraCommand, ErrorOr<Success>>
{
    ...
    public SyncUserStoryToJiraCommandHandler(
    ...
    public async Task<ErrorOr<Success>> HandleAsync(SyncUserStoryToJiraCommand request, CancellationToken cancellationToken = default)
    {
        ...
        if (story.Status == UserStoryStatus.Synced)
        {
            return Errors.UserStories.AlreadySynced;
        }

        var jiraKey = await _jiraSyncService.PushStoryToJiraAsync(user.TenantId.Value, story, cancellationToken);

        if (!string.IsNullOrEmpty(jiraKey))
        {
            story.MarkAsSyncedToJira(jiraKey);
        ...
        {
            return Errors.UserStories.JiraSyncFailed;
        }
    }
}
